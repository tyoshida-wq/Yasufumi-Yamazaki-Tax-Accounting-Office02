...
- **進捗管理ページに2つのモード実装**:
  - **リアルタイムモード**: 現在処理中のタスクの進捗をリアルタイム監視
  - **ログ履歴モード**: 過去タスク（最新50件）のログを閲覧・コピー、日付フィルタリング対応
...
| `GET` | `/api/tasks` | タスク一覧取得（limit指定可、議事録有無フラグ付き、日付フィルタ対応） |
...
- **最新デプロイ**: `33152bf` / `331cdcff`（2026-02-04）
- **ステータス**: ✅ 稼働中
- **最近の改善**: 
  - **進捗管理のログ履歴機能**: タブ切替でリアルタイム/ログ履歴モードを実装
    - リアルタイムモード: 現在処理中のタスクの進捗を監視
    - ログ履歴モード: 過去タスク（最新50件）のログを閲覧・コピー
    - 日付フィルタリング: from/to日付範囲で絞り込み
    - ログコピー機能: ワンクリックでクリップボードにコピー
