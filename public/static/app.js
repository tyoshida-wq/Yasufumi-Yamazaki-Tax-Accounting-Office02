...
function createMeetingCard(task) {
  ...
  const statusBadge = task.status === 'transcribed' || task.status === 'completed'
  ...
  return `
    <div class="meeting-card">
      ...
      <div class="flex gap-2 mt-3">
        ...
        <button class="delete-button bg-red-100 hover:bg-red-200 text-red-700 font-semibold py-2 px-4 rounded-lg" data-task-id="${task.id}">
...
async function loadAdminErrors() {
  ...
  try {
    ...
    errorList.innerHTML = '<div class="text-center text-red-500 py-8">エラーログの読み込みに失敗しました</div>'
  }
}
